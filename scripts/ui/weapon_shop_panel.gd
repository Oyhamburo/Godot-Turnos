...
##
## Panel de la Herrería: tienda de armas con inventario personal integrado.
...
const C_BG_DARK   := Color(0.06, 0.07, 0.12, 0.96)
...
func _refresh_abilities(weapon: WeaponData) -> void:
	...
	for abi in weapon.abilities:
		...
		lbl.text = "• %s: %s %d%% %s" % [abi_name, dmg_txt, int(hit * 100), rng_txt]
		lbl.add_theme_font_size_override("font_size", 12)
		lbl.add_theme_color_override("font_color", C_TEXT)
...
func _on_comprar() -> void:
	...
	_mostrar_mensaje("¡%s comprada!" % _selected_weapon.display_name, true)
	_refresh_gold()
